				...
				<div className="w-full max-w-md text-center">
					<div className="bg-gray-900 rounded-2xl border border-gray-800 p-10">
						...
						<h1 className="text-xl font-bold text-white mb-2">
							Sign in to get Vendor Proof
						</h1>
						<p className="text-gray-400 text-sm mb-8 leading-relaxed">
							Your Vendor Proof certificate is tied to your company account.
							Sign in or create a free account first, then complete your
							purchase.
						</p>
						...
						<p className="text-gray-600 text-xs mt-6">
							Free account · No credit card needed to register
						</p>
					</div>
				</div>
			</main>
				...
				<div className="w-full max-w-md text-center">
					<div className="bg-gray-900 rounded-2xl border border-gray-800 p-10">
						...
						<p className="text-gray-400 text-sm mb-8 leading-relaxed">
							...
							visible to procurement agencies.
						</p>
						<div className="space-y-3">
							<Link
								...
								onClick={async () => {
									try {
										const res = await fetch("/api/checkout", {
											...
											headers: { "Content-Type": "application/json" },
											body: JSON.stringify({
												productType: "vendor_active_monthly",
											}),
										});
								...
								onClick={async () => {
									try {
										const res = await fetch("/api/checkout", {
											...
											headers: { "Content-Type": "application/json" },
											body: JSON.stringify({
												productType: "vendor_active_annual",
											}),
										});
										const data = await res.json();
										if (data.url) window.location.href = data.url;
