...
function scoreBar(value: number, max = 100) {
	...
	const color =
		pct >= 70 ? "bg-[#10b981]" : pct >= 40 ? "bg-yellow-400" : "bg-red-400";
	return (
		<div className="flex flex-col items-center gap-1">
			<span
				className={`font-bold text-base ${pct >= 70 ? "text-[#10b981]" : pct >= 40 ? "text-yellow-600" : "text-red-500"}`}
			>
				{value}
			</span>
...
function riskBadge(signal: string) {
	...
	if (signal === "WATCH")
		return (
			<span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-yellow-50 text-yellow-700">
				⚠ Watch
			</span>
		);
	if (signal === "FLAGGED")
		return (
			<span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-orange-50 text-orange-700">
				⚠ Flagged
			</span>
		);
	if (signal === "CRITICAL")
...
function depthBadge(d: string) {
	...
	return (
		<span
			className={`text-xs font-semibold uppercase ${map[d] ?? "text-[#94a3b8]"}`}
		>
			{d}
		</span>
	);
...
function ClaimNudge({ slug }: { slug: string }) {
	return (
		<div className="flex flex-col items-center gap-1">
			<span className="text-[#94a3b8] text-xs">Not available</span>
			<a
				href={`/vendors/${slug}`}
				className="text-[10px] font-semibold text-[#10b981] hover:underline whitespace-nowrap"
			>
				Claim profile →
			</a>
		</div>
...
function renderProfileCell(key: keyof MarketplaceVendor, value: unknown) {
	...
	if (key === "verified")
		return value ? (
			<span className="text-[#10b981] font-semibold text-sm">✓ Verified</span>
		) : (
			<span className="text-[#94a3b8] text-sm">Not verified</span>
		);
	if (key === "claimed")
		return value ? (
			<span className="text-[#10b981] font-semibold text-sm">✓ Claimed</span>
		) : (
	...
	if (key === "scan_status") {
		...
		if (s === "COMPLETE")
			return (
				<span className="text-[#10b981] font-semibold text-sm">✓ Scanned</span>
			);
		if (s === "SCANNING" || s === "QUEUED")
	...
	if (key === "domain" || key === "website") {
		...
		return (
			<a
				...
				className="text-[#10b981] text-sm hover:underline truncate max-w-[140px] block"
			>
				{url}
			</a>
		);
	...
	if (key === "short_description")
		return (
			<span className="text-xs text-[#64748b] line-clamp-3">
				{String(value)}
			</span>
		);
...
function SectionRow({ label, colCount }: { label: string; colCount: number }) {
	return (
		<tr className="bg-[#f1f5f9]">
			<td
				colSpan={colCount + 1}
				className="px-6 py-2 text-xs font-bold text-[#64748b] uppercase tracking-widest"
			>
				{label}
			</td>
		</tr>
...
export default function ComparePage() {
	...
	const rowClass = (i: number) =>
		`border-b border-[#f1f5f9] ${i % 2 === 0 ? "bg-white" : "bg-[#fafbfc]"}`;
	const labelTd =
		"px-6 py-4 text-xs font-semibold text-[#64748b] uppercase tracking-wide";
			...
			<section className="py-16 px-6 bg-[#0f172a] text-white">
				<div className="max-w-[1200px] mx-auto text-center">
					...
					<p className="text-xl text-[#94a3b8]">
						Side-by-side comparison of up to 4 vendors — profile, trust scores,
						compliance, procurement readiness, and more.
					</p>
				</div>
			</section>
				...
				<div className="max-w-[1200px] mx-auto">
					<div className="relative mb-6">
						<input
							...
							placeholder={
								selected.length >= 4
									? "Maximum 4 vendors selected"
									: "Search vendors to compare (min 2, max 4)…"
							}
							value={search}
							...
							<div className="absolute top-full left-0 right-0 bg-white border border-[#e2e8f0] rounded-xl mt-1 shadow-lg max-h-[300px] overflow-y-auto z-20">
								{results.map((v) => (
									...
									>
										...
										{v.verified && (
											<span className="ml-2 text-xs text-[#10b981] font-semibold">
												✓ Verified
											</span>
										)}
									</button>
					...
					<div className="flex flex-wrap gap-3 mb-4">
						{selected.map((v) => (
							<div
								key={v.id}
								className="flex items-center gap-2 bg-white border border-[#10b981] rounded-xl px-4 py-2"
							>
						...
						{selected.length < 2 && (
							<div className="flex items-center gap-2 border-2 border-dashed border-[#e2e8f0] rounded-xl px-4 py-2 text-[#94a3b8] text-sm">
								Add {2 - selected.length} more vendor
								{2 - selected.length > 1 ? "s" : ""} to compare
							</div>
						)}
						{loading && (
				...
				<section className="px-6 pb-16">
					<div className="max-w-[1200px] mx-auto">
							...
							<table className="w-full text-sm">
								<thead>
									<tr className="border-b border-[#e2e8f0] bg-[#f8fafc]">
										<th className="text-left px-6 py-4 text-xs font-semibold text-[#64748b] uppercase tracking-wide w-44">
											Field
										</th>
										{selected.map((v) => (
											<th key={v.id} className="text-center px-6 py-4">
												<Link
													href={`/vendors/${v.slug || v.seo_slug || v.id}`}
								...
								<tbody>
									...
									{[
										...
										{
											key: "procurement_interest_score",
											label: "Procurement Interest",
										},
									].map(({ key, label }, i) => (
										<tr key={key} className={rowClass(i)}>
											<td className={labelTd}>{label}</td>
									...
									<tr className={rowClass(4)}>
										...
										{selected.map((v) => {
											...
											return (
												<td key={v.id} className={td}>
													{s ? (
														scoreBar(Math.round(s.confidence_score))
													) : !v.claimed ? (
														<ClaimNudge slug={v.slug || v.id} />
													) : (
														<span className="text-[#94a3b8] text-xs">—</span>
													...
													{secs && secs.length > 0 ? (
														<div className="flex flex-wrap gap-1 justify-center">
															{secs.map((s) => (
																<span
																	key={s}
																	className="inline-block px-2 py-0.5 bg-[#f1f5f9] text-[#0f172a] rounded-full text-xs"
																>
																	{s}
																</span>
															))}
						...
						<p className="text-xs text-[#94a3b8] mt-4 text-center">
							Data sourced from BOOPPA Vendor Network. Scores reflect claimed
							&amp; active vendors only.{" "}
							<Link href="/vendors" className="text-[#10b981] hover:underline">
								Browse all vendors →
							</Link>
						</p>
					</div>
			...
			{!showTable && (
				<section className="py-20 px-6 text-center">
					<p className="text-[#94a3b8] text-lg">
						Search and add at least 2 vendors above to start comparing.
					</p>
					<Link
						href="/vendors"
						className="mt-4 inline-block text-[#10b981] font-semibold hover:underline"
					>
