...
function renderCell(key: keyof MarketplaceVendor, value: unknown) {
	...
	if (key === "verified") {
		return value ? (
			<span className="inline-flex items-center gap-1 text-[#10b981] font-semibold text-sm">
				✓ Verified
			</span>
		) : (
			...
			<section className="py-16 px-6 bg-[#0f172a] text-white">
				<div className="max-w-[1200px] mx-auto text-center">
					...
					<p className="text-xl text-[#94a3b8]">
						Side-by-side comparison of up to 4 vendors — trust scores,
						compliance tier, entity data, and more.
					</p>
				</div>
			</section>
								...
								{results.map((v) => (
									...
									>
										...
										{v.trust_score !== undefined && v.trust_score !== null && (
											<span
												className={`ml-2 text-xs font-bold ${scoreColor(v.trust_score)}`}
											>
												{v.trust_score}/100
											</span>
										)}
					...
					<div className="flex flex-wrap gap-3 mb-6">
						{selected.map((v) => (
							...
							>
								...
								{v.trust_score !== undefined && v.trust_score !== null && (
									<span
										className={`text-xs font-bold ${scoreColor(v.trust_score)}`}
									>
										{v.trust_score}/100
									</span>
								)}
				...
				<section className="px-6 pb-16">
					<div className="max-w-[1200px] mx-auto">
							...
							<table className="w-full text-sm">
								<thead>
									<tr className="border-b border-[#e2e8f0] bg-[#f8fafc]">
										<th className="text-left px-6 py-4 text-xs font-semibold text-[#64748b] uppercase tracking-wide w-40">
											Field
										</th>
										{selected.map((v) => (
								...
								<tbody>
									{ROWS.map(({ key, label }, i) => {
										...
										return (
											<tr
												key={key}
												className={`border-b border-[#f1f5f9] ${i % 2 === 0 ? "bg-white" : "bg-[#fafbfc]"}`}
											>
												<td className="px-6 py-4 text-xs font-semibold text-[#64748b] uppercase tracking-wide">
													{label}
												</td>
												{selected.map((v) => (
										...
										<thead>
											<tr className="border-b border-[#e2e8f0] bg-[#f8fafc]">
												...
												{selected.map((v) => (
													<th
														key={v.id}
														className="text-center px-6 py-4 font-bold text-[#0f172a] text-sm"
													>
														{v.company_name}
													</th>
												))}
													...
													>
														...
														{selected.map((v) => {
															...
															return (
																<td
																	key={v.id}
																	className="px-6 py-4 text-center text-sm text-[#0f172a]"
																>
						...
						<p className="text-xs text-[#94a3b8] mt-4 text-center">
							Data sourced from BOOPPA Vendor Network. Last updated at time of
							profile scan.{" "}
