...
export default function OpportunitiesPage() {
	...
	const lastFetched = tenders[0]?.last_fetched_at
		? new Date(tenders[0].last_fetched_at).toLocaleString("en-SG", {
				...
				<div className="mx-auto max-w-5xl">
					<div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
						<div>
							...
							<p className="mt-2 text-white/50 text-sm">
								Singapore government procurement opportunities, refreshed every
								30 minutes.
							</p>
						</div>
						{lastFetched && (
					...
					{authed && !isVendorProof && (
						<div className="mt-6 rounded-xl bg-amber-500/10 border border-amber-500/20 px-5 py-4 text-sm text-amber-300">
							Upgrade to <strong>Vendor Proof</strong> to stand out to agencies
							on these tenders.{" "}
							<Link
								href="/vendor-proof"
								className="underline hover:no-underline"
					...
					{!authed && (
						<div className="mt-6 rounded-xl bg-white/5 border border-white/10 px-5 py-4 text-sm text-white/60">
							<Link
								href="/trial"
								className="text-[#10b981] underline hover:no-underline"
							>
								Create a free account
							</Link>{" "}
							to see your match score and prepare your RFP for any tender.
						</div>
					)}
				</div>
			</section>
								...
								<div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
									<div className="flex-1 min-w-0">
										...
										<div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-white/40">
											...
											{tender.closing_date && (
												<span
													className={`flex items-center gap-1 ${urgencyColor(days)}`}
												>
											...
											{tender.estimated_value && (
												<span>
													Est. SGD {tender.estimated_value.toLocaleString()}
												</span>
											)}
										</div>
									</div>
									...
									<div className="flex flex-shrink-0 items-center gap-2">
										...
										<Link
											href={
												authed
													? `/rfp-acceleration?tender=${tender.tender_no}`
													: "/trial"
											}
										...
										>
											<FileText className="h-3 w-3" /> Prepare RFP
										</Link>
									</div>
								</div>
							</div>
