          ...
          <div className="grid lg:grid-cols-3 gap-6">
            {pricingTiers.map((tier) => (
              <div key={tier.productType} className={`${tier.bgColor} p-6 rounded-xl border ${tier.borderColor} flex flex-col` }>
                ...
                <Link href={`${(process.env.NEXT_PUBLIC_API_BASE ?? 'https://api.booppa.io')}/api/stripe/checkout?product=${tier.productType}`} className="mt-auto inline-flex items-center justify-center rounded-lg bg-booppa-green px-4 py-2 text-black font-semibold">Buy Now</Link>
              </div>
            ))}
          </div>
        </section>
