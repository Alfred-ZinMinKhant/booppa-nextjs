          ...
          <div className="grid lg:grid-cols-3 gap-6">
            {pricingTiers.map((tier) => (
              <div key={tier.productType} className={`${tier.bgColor} p-6 rounded-xl border ${tier.borderColor} flex flex-col`}>
                ...
                <div className={`text-3xl font-black mb-2 ${tier.priceColor}`}>{tier.price}</div>
                <p className="text-gray-400 mb-4">{tier.subtitle}</p>
                <ul className="text-gray-300 mb-6 space-y-2">
                  {tier.features.map((f) => (
                    <li key={f} className="flex items-start">
                      <Check className="w-5 h-5 text-green-400 mr-3 mt-0.5 flex-shrink-0" />{f}
                    </li>
                  ))}
                </ul>
                <Link
                  href={`/notarization?product=${tier.productType}`}
                  className="mt-auto inline-flex items-center justify-center rounded-lg bg-emerald-500 hover:bg-emerald-600 px-4 py-2 text-black font-semibold transition"
                >
                  Buy Now
                </Link>
              </div>
        ...
        <section className="mb-12">
          ...
          <div className="mt-6">
            <p className="text-gray-400">
              Need higher volumes or integration? For custom notarization volumes, automated workflows, or API integrations, please{' '}
              <Link href="/demo" className="text-emerald-400 hover:underline">contact us</Link>.
            </p>
          </div>
        </section>
      </div>
