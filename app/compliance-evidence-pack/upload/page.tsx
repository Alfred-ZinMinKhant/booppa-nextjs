...
export default function ComplianceEvidencePackUploadPage() {
  ...
  useEffect(() => {
    if (!email) return
    refreshState(email)
    // Poll while there are pending reports waiting on tx_hash
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (!data.skip_checkout) {
        // Means no credit was applied — surface clearly
    ...
    <main className="bg-white min-h-screen py-16 px-6">
      <div className="container max-w-4xl mx-auto">
        <div className="mb-10">
          ...
          <h1 className="text-3xl lg:text-4xl font-black text-[#0f172a] mb-3">Upload your compliance documents</h1>
          <p className="text-[#64748b] text-lg leading-relaxed">
            Each document you upload is hashed with SHA-256 and anchored on {POLYGON_EXPLORER_HOST}.
            Once you've uploaded all your documents (or click "Generate now"), we'll compile your
            9-section regulator-ready Cover Sheet PDF.
          </p>
        </div>
        ...
        {/* Email field (only shown if not auto-detected) */}
        {!email && (
          <div className="mb-6">
        ...
        <div className="bg-[#f8fafc] border-2 border-[#10b981] rounded-2xl p-6">
          ...
          <p className="text-sm text-[#64748b] mb-4">
            ...
            {remaining === 0 && uploadedCount > 0 && (
              <span className="block mt-2 text-[#0f172a] font-medium">
                All credits redeemed — your cover sheet may already be on its way to your inbox.
              </span>
            )}
          </p>
          <button
