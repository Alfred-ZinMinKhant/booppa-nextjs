	...
	return (
		<div className="relative flex items-center justify-center">
			...
			<div className="absolute inset-0 flex flex-col items-center justify-center">
				<span className={`text-3xl font-bold ${probColor(value)}`}>
					{value}%
				</span>
...
function TenderCheckContent() {
	...
	const startCheckout = async (productType: string) => {
		...
		try {
			...
			if (res.ok && data.checkout_url) {
				...
				router.push(`/pricing?product=${productType}&upgrade=1`);
			}
		} catch {
			router.push(`/pricing?product=${productType}&upgrade=1`);
		} finally {
	...
	async function runCheck(tenderNo: string) {
		...
		try {
			...
			if (!res.ok) {
				setError("Could not reach the tender service — please try again.");
				return;
			}
			...
			<div className="max-w-5xl mx-auto space-y-8">
				...
				<div className="border-b border-neutral-800 pb-6">
					...
					<p className="text-neutral-400 text-sm mt-1">
						Enter a GeBIZ tender number to estimate your chances and see what
						upgrading your profile would unlock.
					</p>
				</div>
				...
				{error && (
					<div className="rounded-lg border border-red-500/20 bg-red-500/5 px-4 py-3 space-y-1">
						...
						<p className="text-xs text-neutral-500 pl-7">
							BOOPPA covers open ITQ/ITT tenders and recently awarded contracts
							from GeBIZ. Historical or niche procurement opportunities may not
							be indexed yet.
						</p>
					</div>
				)}
							...
							<div className="rounded-xl border border-neutral-800 bg-neutral-900 overflow-hidden">
								<div className="px-6 py-4 border-b border-neutral-800">
									<h3 className="text-sm font-semibold text-white">
										Your Profile vs Sector Winners
									</h3>
									<p className="text-xs text-neutral-500 mt-0.5">
										How you compare against vendors who typically win in this
										sector
									</p>
								</div>
									...
									<div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-5 space-y-4">
										<div className="flex items-start justify-between">
											<div>
												...
												<p className="text-xs text-neutral-400">
													{result.projections.rfpComplete.simulates}
												</p>
											</div>
											<div className="text-right">
										...
										<div className="h-1.5 w-full rounded-full bg-neutral-800">
											<div
												...
												style={{
													width: `${result.projections.rfpComplete.probability}%`,
												}}
											/>
										</div>
										<button
											type="button"
											onClick={() => startCheckout("rfp_complete")}
											disabled={checkingOut === "rfp_complete"}
											className="flex items-center justify-center gap-2 w-full py-2 rounded-lg bg-emerald-600 text-white text-sm font-medium hover:bg-emerald-700 transition disabled:opacity-60"
										>
											{checkingOut === "rfp_complete" ? (
												"Redirecting…"
											) : (
												<>
													Upgrade to RFP Complete{" "}
													<ArrowRight className="h-3.5 w-3.5" />
												</>
											)}
										</button>
				...
				<div className="mt-8 mx-auto max-w-2xl px-4 pb-12">
					<div className="bg-neutral-900 border border-neutral-700 rounded-xl p-5">
						<p className="text-xs text-neutral-400 leading-relaxed">
							...
							Tender Win Probability scores are non-predictive estimates
							generated for informational purposes only. They do not guarantee,
							predict, or influence the outcome of any tender award. BOOPPA is
							not affiliated with, endorsed by, or connected to GeBIZ, the
							Ministry of Trade and Industry (MTI), the Personal Data Protection
							Commission (PDPC), or any Singapore government agency. Results are
							based on publicly available data and internal scoring models. For
							official tender requirements, refer to the relevant government
							portal.
						</p>
					</div>
				</div>
