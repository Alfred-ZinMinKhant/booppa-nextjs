...
export default function AdminDashboard() {
	...
	useEffect(() => {
		Promise.all([
			fetch(`/api/v1${endpoints.features.list}`, {
				credentials: "include",
			}).then((r) => {
				if (r.status === 401) {
			...
			fetch(`/api/v1${endpoints.features.metrics}`, {
				credentials: "include",
			}).then((r) => (r.ok ? r.json() : null)),
			fetch(`/api/v1${endpoints.funnel.summary}`, {
				credentials: "include",
			}).then((r) => (r.ok ? r.json() : null)),
		])
	...
	const toggleFlag = async (flagName: string, currentValue: boolean) => {
		try {
			...
			if (res.ok) {
				setFlags((prev) =>
					prev.map((f) =>
						f.flag_name === flagName ? { ...f, enabled: !currentValue } : f,
					),
				);
			}
		} catch {
							...
							<div className="bg-white p-6 rounded-2xl border border-[#e2e8f0]">
								...
								<div className="mt-2 w-full bg-[#e2e8f0] rounded-full h-2">
									<div
										...
										style={{
											width: `${Math.min(100, (metrics.vendors / 500) * 100)}%`,
										}}
									/>
								</div>
							</div>
							...
							<div className="divide-y divide-[#e2e8f0]">
								{flags.map((flag) => (
									...
									>
										...
										>
											<span
												className={`inline-block h-4 w-4 rounded-full bg-white transition-transform ${flag.enabled ? "translate-x-6" : "translate-x-1"}`}
											/>
										</button>
									</div>
								))}
								...
								<div className="p-6 space-y-3">
									{funnel.map((stage) => {
										...
										return (
											...
											>
												<div className="w-24 text-xs text-[#64748b] text-right">
													{stage.stage}
												</div>
