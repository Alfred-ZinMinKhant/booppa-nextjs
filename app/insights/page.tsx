...
function deriveInsights(
	...
	const avgScore = Math.round(
		entries.reduce((s, e) => s + e.score, 0) / totalVendors,
	);
	const unverifiedCount = entries.filter(
		(e) => e.verification_depth === "UNVERIFIED",
	).length;
...
export default function InsightsPage() {
	...
	useEffect(() => {
		...
		setLoading(true);
		Promise.all([
			fetch(`/api/rankings/leaderboard?${params}`).then((r) =>
				r.ok ? r.json() : { entries: [] },
			),
			fetch(`/api/marketplace/industries`).then((r) =>
				r.ok ? r.json() : { industries: [] },
			),
		])
			.then(([lb, ind]) => {
	...
	return (
		<main className="min-h-screen bg-[#0f172a] text-white">
			...
			<section className="border-b border-white/10 px-6 py-16 lg:px-8">
				<div className="mx-auto max-w-5xl">
					...
					<h1 className="text-3xl lg:text-4xl font-bold mb-3 text-white">
						Procurement &amp; Compliance Insights
					</h1>
					<p className="text-white/50 text-sm max-w-xl">
						Key findings from the BOOPPA vendor network — what the data means,
						where the gaps are, and what you should do about it.
					</p>
				</div>
			</section>
			...
			<section className="px-6 py-12 lg:px-8 border-b border-white/10">
				<div className="mx-auto max-w-5xl">
					<h2 className="text-xs font-semibold uppercase tracking-widest text-white/40 mb-8">
						Key Findings
					</h2>
								...
								{entries.length === 0 ? (
									<div className="rounded-xl border border-dashed border-white/10 py-16 text-center">
										<p className="text-white/30 text-sm">
											No data available for this industry yet.
										</p>
										<Link
											href="/auth/register"
											className="mt-4 inline-block text-[#10b981] text-sm underline hover:no-underline"
										>
										...
										<table className="w-full text-sm">
											<thead>
												<tr className="border-b border-white/10 bg-white/5">
													<th className="text-left px-5 py-3 text-xs text-white/40 font-medium w-12">
														#
													</th>
													<th className="text-left px-5 py-3 text-xs text-white/40 font-medium">
														Company
													</th>
													<th className="text-left px-5 py-3 text-xs text-white/40 font-medium hidden sm:table-cell">
														Industry
													</th>
													<th className="text-left px-5 py-3 text-xs text-white/40 font-medium hidden md:table-cell">
														Verification
													</th>
													<th className="text-right px-5 py-3 text-xs text-white/40 font-medium hidden md:table-cell">
														Evidence
													</th>
											...
											<tbody className="divide-y divide-white/5">
												{entries.map((e, i) => (
													...
													>
														...
														<td className="px-5 py-3 text-white/50 hidden sm:table-cell">
															{e.sector || "—"}
														</td>
														<td
															className={`px-5 py-3 text-xs font-semibold hidden md:table-cell ${DEPTH_COLOR[e.verification_depth] ?? "text-white/40"}`}
														>
														...
														<td className="px-5 py-3 text-right font-bold text-[#10b981]">
															{e.score}
														</td>
													</tr>
												))}
			...
			<section className="px-6 py-16 lg:px-8 border-t border-white/10 text-center">
				<div className="max-w-xl mx-auto">
					...
					<p className="text-white/50 text-sm mb-8">
						Claim your free profile and start building your compliance score
						today.
					</p>
					<Link
						href="/auth/register"
