...
export default function InsightDomePage() {
	...
	useEffect(() => {
		...
		Promise.all([
			fetch(`${config.apiUrl}/api/v1${endpoints.features.metrics}`).then((r) =>
				r.ok ? r.json() : null,
			),
			fetch(`${config.apiUrl}/api/v1${endpoints.funnel.revenue}`).then((r) =>
				r.ok ? r.json() : null,
			),
			fetch(`${config.apiUrl}/api/v1${endpoints.marketplace.industries}`).then(
				(r) => (r.ok ? r.json() : null),
			),
		])
			.then(([m, r, ind]) => {
			...
			<main className="min-h-screen bg-[#f8fafc] flex items-center justify-center">
				<div className="text-center max-w-md">
					...
					<p className="text-[#64748b] mb-6">
						Market intelligence will be available once we reach 500 verified
						vendors.
					</p>
					<Link
						href="/vendors"
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
								...
								<div>
									...
									<div className="space-y-3">
										{Object.entries(revenue.by_product || {}).map(
											([product, amount]: [string, any]) => (
												<div
													key={product}
								...
								<div>
									...
									<div className="space-y-3">
										{Object.entries(revenue.by_type || {}).map(
							...
							{industries.slice(0, 12).map((ind: any) => {
								...
								return (
									<div key={ind.industry} className="flex items-center gap-3">
										...
										<div className="w-12 text-xs text-[#94a3b8] text-right">
											{ind.count}
										</div>
									</div>
								);
